...
Signature.prototype.from_tlv = function (block)
{
    ...
    type = block.read_var_num ();
    if (type != NdnType.SignatureValue)
	throw new Error ('Signature.from_tlv: wrong SignatureValue type');

    len = block.read_var_num ();  // SignatureValue length
    this.value = block.read_array (len);
...
Signature.prototype.to_tlv = function (block)
{
	...
	// Encode SignatureValue fields
	total_len += block.push_var_num (128);
	total_len += block.push_var_num (NdnType.SignatureValue);
...
{
    if (this.type == Signature.DigestSha256)
	return 32 + Block.var_num_length (32) + Block.var_num_length (NdnType.SignatureValue);
    else if (this.type == Signature.SignatureSha256WithRsa)
	return 128 + Block.var_num_length (128) + Block.var_num_length (NdnType.SignatureValue);
    else
