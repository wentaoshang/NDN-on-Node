...
/**
 * name and content cannot be null. signedinfo is optional.
 */
...
/**
 * Initialize SignedInfo with a Key object
 */
SignedInfo.prototype.setFields = function (/* Key */ key) {
    this.publisher = new PublisherPublicKeyDigest(key.publicKeyDigest);
...
SignedInfo.prototype.to_xml = function (encoding) {
	...
	xml += '<Timestamp ccnbencoding="' + encoding + 'Binary">' 
	    + DataUtils.unsignedIntToBigEndian((this.timestamp.msec / 1000) * 4096).toString(encoding).toUpperCase()
	    + '</Timestamp>';
